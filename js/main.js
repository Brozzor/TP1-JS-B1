...
function timerStart(){
  if (quizz.timer > 7950){return;} // permet de supprimer le timer a la fin d'une partie
  // magnifique timer recurssif
  if (quizz.timer >= 0){
    if (quizz.timer < 10){
    document.getElementById("timer").innerText = "00:0" + quizz.timer;
    if (quizz.timer < 5){colorModify('lowTime')}
  }else{
...
function endGame1(){
  ...
  }else if ((quizz.score >= 7) && (level == 1 || level == 2)){
    document.getElementById("question").innerHTML = '<font color="#28a745"><center>C\'est pas mal mais tu peut passer au niveau superieur ;)</center></font>';
